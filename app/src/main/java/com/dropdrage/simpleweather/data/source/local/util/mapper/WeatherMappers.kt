...
import com.dropdrage.simpleweather.data.source.local.cache.model.TemperatureRange
import com.dropdrage.simpleweather.data.source.local.cache.relation.DayToHourWeather
import com.dropdrage.simpleweather.data.source.local.util.WeatherUnitsDeconverter
...
fun List<DayToHourWeather>.toDomainWeather(): DomainWeather = DomainWeather(
...
fun DomainWeather.toDayModels(locationId: Long): List<DayWeatherModel> = dailyWeather.map {
    val date = it.date
...
fun DomainWeather.toHourModels(dayIds: List<Long>): List<HourWeatherModel> =
...
fun CurrentWeatherDto.toDomain(): DomainCurrentWeather = DomainCurrentWeather(
